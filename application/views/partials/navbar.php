    ...
    <div class="scrollbar-inner">
      ...
      <div class="sidenav-header d-flex align-items-center">
        <a class="navbar-brand" href="<?= site_url('assets/pages/dashboards/alternative.html') ?>" target="_blank">
          <img src="<?= site_url('assets/assets/img/brand/logo.png') ?>" class="navbar-brand-img" alt="...">
        </a>
        <div class="ml-auto">
          <!-- Sidenav toggler -->
          ...
          <ul class="navbar-nav">
            ...
            <?php 
              ...
              foreach ($menu->result() as $mn) {
                          ...
                          <ul class="nav nav-sm flex-column">
                            <?php 
                                foreach ($sub_menu->result() as $sb) {?>
                                  <li class="nav-item">
                                    <a href="<?= site_url($sb->link) ?>" class="nav-link"><?= $sb->nama_menu ?></a>
                                  </li>  
                            <?php } ?>       
                          </ul>
                        </div>
                  ...
                  <?php }else{ ?>
                  <li class="nav-item">
                    <a class="nav-link" href="<?= site_url($mn->link) ?>">
                      <i class="ni ni-<?= $mn->icon .' '.$mn->color?>"></i>
                      <span class="nav-link-text"><?= $mn->nama_menu ?></span>
                    </a>
                  </li>    
              ...
              <div class="dropdown-menu dropdown-menu-lg dropdown-menu-dark bg-default dropdown-menu-right">
                <div class="row shortcuts px-4">
                  <?php 
                    $menu = $this->db->get_where('menus', array('sub_menu' => 0, 'level' => $this->session->userdata('level')));
                    foreach ($menu->result() as $mn) {?>
                      <a href="<?= site_url($mn->link) ?>" class="col-4 shortcut-item">
                        <span class="shortcut-media avatar rounded-circle bg-gradient-blue">
                          <i class="ni ni-<?= $mn->icon ?>"></i>
                        </span>
                        <small><?= $mn->nama_menu ?></small>
                      </a>
                    <?php 
                    } 
                  ?>
